...
class MOMERepertoireFixed(MapElitesRepertoire):
    ...
    def _sample_in_masked_pareto_front(
      ...
      random_key: RNGKey,
        ...
        genotype_sample = jax.tree_util.tree_map(
            lambda x: jax.random.choice(random_key,
                                        x,
                                        shape=(1,),
                                        p=p),
    ...
    def sample(self,
        ...
        repertoire_empty = jnp.any(self.fitnesses == -jnp.inf,
                                   axis=-1)
        occupied_cells = jnp.any(~repertoire_empty,
                                 axis=-1)

        p = occupied_cells / jnp.sum(occupied_cells)

        # possible indices - num cells
        indices = jnp.arange(start=0,
                             stop=repertoire_empty.shape[0])

        # choose idx - among indices of cells that are not empty
        random_key, subkey = jax.random.split(random_key)
        cells_idx = jax.random.choice(subkey,
                                      indices,
                                      shape=(num_samples,),
                                      p=p)
    ...
    def _update_masked_pareto_front(
      ...
      new_mask: Mask,
        ...
        # gather all data
        cat_mask = jnp.concatenate([mask, new_mask],
                                   axis=-1)
        cat_fitnesses = jnp.concatenate(
            [pareto_front_fitnesses, new_batch_of_fitnesses],
            axis=0
        )
        ...
        new_front_descriptors = jnp.take(cat_descriptors,
                                         indices,
                                         axis=0)

        # compute new mask
        num_front_elements = jnp.sum(cat_bool_front)
        new_mask_indices = jnp.arange(start=0,
                                      stop=batch_size + pareto_front_len)
        new_mask_indices = (num_front_elements - new_mask_indices) > 0

        new_mask = jnp.where(
            new_mask_indices,
            jnp.ones(shape=batch_size + pareto_front_len,
                     dtype=bool),
            jnp.zeros(shape=batch_size + pareto_front_len,
                      dtype=bool),
        )

        fitness_mask = jnp.repeat(
            jnp.expand_dims(new_mask,
                            axis=-1),
            num_criteria,
            axis=-1
        )
        ...
        new_front_genotypes = jax.tree_util.tree_map(
            lambda x: x * new_mask_indices[0],
            new_front_genotypes
        )
        new_front_genotypes = jax.tree_util.tree_map(
            lambda x: x[:front_size],
            new_front_genotypes
        )
    ...
    def add(
      ...
      batch_of_fitnesses: Fitness,
        ...
        batch_of_indices = get_cells_indices(batch_of_descriptors,
                                             self.centroids)
        batch_of_indices = jnp.expand_dims(batch_of_indices,
                                           axis=-1)

        def _add_one(
          carry: MOMERepertoireFixed,
          data: Tuple[Genotype, Descriptor, Fitness, jnp.ndarray],
        ) -> Tuple[MOMERepertoireFixed, Any]:
            # unwrap data
            ...
            cell_genotype = jax.tree_util.tree_map(lambda x: x[index][0],
                                                   carry.genotypes)
            cell_fitness = carry.fitnesses[index][0]
            cell_descriptor = carry.descriptors[index][0]
            cell_mask = jnp.any(cell_fitness == -jnp.inf,
                                axis=-1)

            new_genotypes = jax.tree_util.tree_map(lambda x: jnp.expand_dims(x,
                                                                             axis=0),
                                                   genotype)
            ...
            (
                ...
                mask=cell_mask,
                new_batch_of_fitnesses=jnp.expand_dims(fitness,
                                                       axis=0),
                new_batch_of_genotypes=new_genotypes,
                new_batch_of_descriptors=jnp.expand_dims(descriptors,
                                                         axis=0),
                new_mask=jnp.zeros(shape=(1,),
                                   dtype=bool),
            )

            # update cell fitness
            cell_fitness = cell_fitness - jnp.inf * jnp.expand_dims(cell_mask,
                                                                    axis=-1)
    ...
    def init(  # type: ignore
      ...
      extra_scores: ExtraScores=None,
        ...
        default_genotypes = jax.tree_util.tree_map(
            lambda x: jnp.zeros(
                shape=(
                      ...
                      + x.shape[1:]
                # Use [1:] as first dimension is batch size (so ignore this)
            ),
            genotypes,
        )
        ...
        repertoire = MOMERepertoireFixed(  # type: ignore
        ...
        new_repertoire = repertoire.add(genotypes,
                                        descriptors,
                                        fitnesses)
    ...
    def compute_global_pareto_front(
      self,
        ...
        """
        fitnesses = jnp.concatenate(self.fitnesses,
                                    axis=0)
        mask = jnp.any(fitnesses == -jnp.inf,
                       axis=-1)
        pareto_mask = compute_masked_pareto_front(fitnesses,
                                                  mask)
    ...
    def get_best_individuals(self) -> Tuple[Genotype, Fitness, jnp.ndarray]:
        ...
        fitnesses = jnp.concatenate(self.fitnesses,
                                    axis=0)
        best_individuals = jnp.argmax(fitnesses,
                                      axis=0)
        best_fitnesses = jnp.take(fitnesses,
                                  best_individuals,
                                  axis=0)
